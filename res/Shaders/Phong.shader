...
void main()
{
    ...
    // ================================
    // Specular highlights simulate the reflection of light on a shiny surface.
    // It is based on the Phong reflection model: I_s = k_s * I_L * max(0, R · V)^s
    // Where:
    // - k_s = uSpecularIntensity (specular reflection coefficient)
    // - I_L = uLight.Colour (light colour intensity)
    // - R = reflection vector, computed as:
    //     R = 2(N · L)N - L
    // - V = view (camera) direction
    // - s = uShininess (shininess exponent, higher values = sharper highlights)
